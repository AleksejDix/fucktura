    ...
    <header class="border-b border-gray-200 px-3 py-2 space-y-2">
      ...
      <div v-if="store.statusPillsForView.length > 0" class="flex flex-wrap gap-1">
        <button
          ...
          :class="!store.activeStatusPill ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'"
        >{{ $t('Any status') }}</button>
        <button
          ...
          class="text-[10px] px-2 py-0.5 rounded-sm transition-colors"
          :class="store.activeStatusPill === s ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'"
        >{{ $t(s) }}</button>
      </div>
    </header>
    <div class="flex-1 overflow-y-auto">
      ...
      >
        <div class="flex items-baseline justify-between text-[12px]">
          <span class="flex items-center gap-1.5 font-medium truncate" :class="doc.number === store.activeDocumentNumber ? 'text-white' : store.isOverdue(doc) ? 'text-red-600' : 'text-gray-900'">
            <span class="shrink-0 w-1.5 h-1.5 inline-block" :class="doc.number === store.activeDocumentNumber ? 'bg-white' : store.isOverdue(doc) ? 'bg-red-500' : dotClass[doc.type]" />
            {{ doc.recipient.company || doc.recipient.name || $t('No client') }}
          </span>
          <span class="shrink-0 ml-2 text-[11px]" :class="doc.number === store.activeDocumentNumber ? 'text-gray-400' : store.isOverdue(doc) ? 'text-red-400' : 'text-gray-400'">{{ formatDate(dueDateField(doc) || doc.meta.date) }}</span>
        </div>
      ...
      <p v-if="store.filteredDocuments.length === 0" class="px-3 py-8 text-xs text-gray-300 text-center">
        {{ $t('No documents') }}
      </p>
    </div>
