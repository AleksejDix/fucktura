...
export function isSender(v: unknown): v is Sender {
  ...
  if (!isStr(v.company) || !isStr(v.street) || !isStr(v.zip) || !isStr(v.city) || !isStr(v.country))
    return false;
  if (!isStr(v.website) || !isStr(v.uid) || !isStr(v.contact) || !isStr(v.contactEmail))
    return false;
...
export function isClient(v: unknown): v is Client {
  ...
  if (!isStr(v.company) || !isStr(v.street) || !isStr(v.zip) || !isStr(v.city) || !isStr(v.country))
    return false;
  if (v.positions !== undefined && (!isArr(v.positions) || !v.positions.every(isClientPosition)))
    return false;
  return true;
}
...
export function isPosition(v: unknown): v is Position {
  if (!isObj(v)) return false;
  return (
    isStr(v.id) && isStr(v.description) && isStr(v.code) && isStr(v.unit) && isNum(v.defaultPrice)
  );
}
